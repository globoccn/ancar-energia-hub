...
const badgeClass: Record<ShoppingStatus, string> = {
  ...
  atencao: "bg-[var(--accent-yellow)]/15 text-[var(--accent-yellow)] border-[var(--accent-yellow)]/30",
...
export function StatusBadge({ status }: { status: ShoppingStatus }) {
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-md border px-2 py-0.5 text-[11px] font-medium", badgeClass[status])}>
