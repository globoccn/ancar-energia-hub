...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Visão Geral — Ancar" },
      {
        name: "description",
        content: "Visão geral do portfólio energético e ESG dos shoppings Ancar.",
      },
    ],
  }),
...
function OverviewPage() {
  ...
  useEffect(() => {
    let alive = true;
    dashboardService.getPortfolioOverview().then((d) => {
      if (alive) setData(d);
    });
    return () => {
      alive = false;
      ...
      <PageHeader
        ...
        right={
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="inline-flex h-2 w-2 rounded-full bg-[var(--accent-cyan)]" /> Atualizado{" "}
            ...
            <div>
              ...
              <div className="mt-1 flex gap-4 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-[var(--accent-green)]" /> Eficiência
                  (kW/TR)
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="h-2 w-2 rounded-full bg-[var(--accent-blue)]" /> Consumo (MWh)
                </span>
              </div>
            </div>
            ...
            <ResponsiveContainer>
              <ComposedChart data={data.series}>
                ...
                <Line
                  ...
                  strokeWidth={2}
                  dot={{ r: 2.5, fill: "var(--accent-green)" }}
                />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
            ...
            {data.ranking.slice(0, 8).map((r) => (
              ...
              >
                ...
                <div className="hidden h-1.5 w-24 overflow-hidden rounded-full bg-muted/50 sm:block">
                  <div
                    className="h-full rounded-full bg-gradient-to-r from-[var(--accent-green)] to-[var(--accent-cyan)]"
                    style={{ width: `${Math.min(100, (r.value / 1.2) * 100)}%` }}
                  />
                </div>
                <span className="metric-value w-12 text-right text-xs">
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-12">
        <div className="panel p-4 xl:col-span-5">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {data.shoppings.slice(0, 6).map((s) => (
            ...
            {data.insights.map((i) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {i.detail && (
                    <div className="mt-0.5 text-[11px] text-[var(--accent-cyan)]">{i.detail}</div>
                  )}
                </div>
