...
const PALETTE = ["var(--accent-cyan)", "var(--accent-green)", "var(--accent-blue)", "var(--accent-yellow)", "var(--accent-purple)"];
...
function AnalisesPage() {
  ...
  const [series, setSeries] = useState<{ id: string; code: string; series: { time: string; consumo: number; eficiencia: number }[] }[]>([]);
  ...
  const combined = useMemo(() => {
    ...
    return times.map((t, i) => {
      ...
      series.forEach((s) => { row[s.code] = s.series[i][metric]; });
      return row;
    });
  }, [series, metric]);

  const scatterData = shoppings.map((s) => ({ x: s.thermalLoadTR ?? 0, y: s.efficiencyKWTR, z: s.powerKW, name: s.code }));
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Análises" subtitle="Compare múltiplos shoppings e métricas em qualquer período" />
      ...
      <div className="panel flex flex-wrap items-center gap-3 p-3">
        ...
        <div className="ml-auto inline-flex rounded-md border border-border/60 bg-card/60 p-0.5">
          {["consumo", "eficiencia"].map((m) => (
            <button key={m} onClick={() => setMetric(m as "consumo" | "eficiencia")}
              className={`rounded px-3 py-1 text-xs capitalize ${metric === m ? "bg-muted text-foreground" : "text-muted-foreground"}`}>{m}</button>
          ))}
        </div>
        <select className="h-9 rounded-md border border-border/60 bg-card/60 px-2 text-sm">
          {["24h", "7 dias", "30 dias", "Personalizado"].map((p) => <option key={p}>{p}</option>)}
        </select>
      </div>

      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <div className="panel p-4">
          <h3 className="mb-3 text-sm font-semibold">Comparação — {metric === "consumo" ? "Consumo" : "Eficiência"}</h3>
          <div className="h-72">
            <ResponsiveContainer>
              <LineChart data={combined}>
                ...
                {series.map((s, i) => (
                  <Line key={s.id} type="monotone" dataKey={s.code} stroke={PALETTE[i % PALETTE.length]} strokeWidth={2} dot={false} />
                ))}
              </LineChart>
            </ResponsiveContainer>
          </div>
            ...
            <ResponsiveContainer>
              <BarChart data={combined}>
                ...
                {series.slice(0, 3).map((s, i) => (
                  <Bar key={s.id} dataKey={s.code} fill={PALETTE[i % PALETTE.length]} radius={[3, 3, 0, 0]} />
                ))}
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="h-72">
            <ResponsiveContainer>
              <ScatterChart>
                ...
                <XAxis type="number" dataKey="x" name="Carga TR" stroke="oklch(0.6 0.02 250)" tick={{ fontSize: 11 }} />
                <YAxis type="number" dataKey="y" name="kW/TR" stroke="oklch(0.6 0.02 250)" tick={{ fontSize: 11 }} />
          ...
          <table className="w-full text-xs">
            <thead>
              <tr>
                ...
                {heatmapDays.map((d) => <th key={d} className="p-2 text-center text-muted-foreground">{d}</th>)}
              </tr>
            </thead>
            <tbody>
                  ...
                  {heatmapDays.map((_, di) => {
                    ...
                    return (
                      <td key={di} className="p-1">
                        <div className="h-8 rounded" style={{ background: `oklch(0.5 0.15 220 / ${0.15 + intensity * 0.7})` }} title={`${v} MWh`} />
                      </td>
                    );
                  })}
                </tr>
